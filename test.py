...
def compare_f0_with_measures(f0_ref, f0_test, sr, hop_length, bpm, tolerance=10, y_ref=None, y_test=None):
    """
    ...
    for measure_ref, measure_test in zip(measures_ref, measures_test):
        if len(measure_ref) == 0 or len(measure_test) == 0:
            ...
            measure_differences.append(0)  # 빈 마디는 차이를 0으로 처리
            continue
    ...
    for i in range(min(32, len(normalized_measures_test))):
        ...
        if is_significant_diff:
            ...
            # 시점별 차이의 임계값 설정 (예: 상위 10% 이상인 경우를 큰 차이로 간주)
...
def analyze_guitar_performance(original_file, played_file, bpm):
    ...
    compare_f0_with_measures(f0_ref, f0_test, sr, hop_length=256, bpm=bpm, 
                           tolerance=10, y_ref=origin_y, y_test=played_y)

# 사용 예시
analyze_guitar_performance('audio/26-gp(clean).wav', 'audio/26-record(clean).wav', bpm=92)